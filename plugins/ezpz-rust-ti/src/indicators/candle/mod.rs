...
impl CandleTI {
	...
	fn moving_constant_envelopes_single(&self, constant_model_type: &str, difference: f64) -> PyResult<PyDfStubbed> {
	...
	fn mcginley_dynamic_envelopes_single(&self, difference: f64, previous_mcginley_dynamic: f64) -> PyResult<PyDfStubbed> {
	...
	fn moving_constant_bands_single(&self, constant_model_type: &str, deviation_model: &str, deviation_multiplier: f64) -> PyResult<PyDfStubbed> {
		let values: Vec<f64> = extract_f64_values(self.series.clone())?;
		let constant_type = parse_constant_model_type(constant_model_type)?;
	...
	fn mcginley_dynamic_bands_single(&self, deviation_model: &str, deviation_multiplier: f64, previous_mcginley_dynamic: f64) -> PyResult<PyDfStubbed> {
		let values: Vec<f64> = extract_f64_values(self.series.clone())?;
		let deviation_type = parse_deviation_model(deviation_model)?;
	...
	fn donchian_channels_single(&self, highs: PySeriesStubbed, lows: PySeriesStubbed) -> PyResult<PyDfStubbed> {
		let high_values: Vec<f64> = extract_f64_values(highs)?;
		let low_values: Vec<f64> = extract_f64_values(lows)?;
	...
	fn supertrend_single(&self, highs: PySeriesStubbed, lows: PySeriesStubbed, constant_model_type: &str, multiplier: f64) -> PyResult<PySeriesStubbed> {
		let high_values: Vec<f64> = extract_f64_values(highs)?;
		let low_values: Vec<f64> = extract_f64_values(lows)?;
		let close_values: Vec<f64> = extract_f64_values(self.series.clone())?;
		let constant_type = parse_constant_model_type(constant_model_type)?;
	...
	fn moving_constant_envelopes_bulk(&self, constant_model_type: &str, difference: f64, period: usize) -> PyResult<PyDfStubbed> {
		let values: Vec<f64> = extract_f64_values(self.series.clone())?;
		let constant_type = parse_constant_model_type(constant_model_type)?;
	...
	/// - `upper_envelope`: Vec<f64> - Time series of upper envelope bands
	fn mcginley_dynamic_envelopes_bulk(&self, difference: f64, previous_mcginley_dynamic: f64, period: usize) -> PyResult<PyDfStubbed> {
		let values: Vec<f64> = extract_f64_values(self.series.clone())?;
	...
	fn moving_constant_bands_bulk(&self, constant_model_type: &str, deviation_model: &str, deviation_multiplier: f64, period: usize) -> PyResult<PyDfStubbed> {
		let values: Vec<f64> = extract_f64_values(self.series.clone())?;
		let constant_type = parse_constant_model_type(constant_model_type)?;
		let deviation_type = parse_deviation_model(deviation_model)?;
	...
	fn donchian_channels_bulk(&self, highs: PySeriesStubbed, lows: PySeriesStubbed, period: usize) -> PyResult<PyDfStubbed> {
